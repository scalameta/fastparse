...
with Compat210Component {
  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) with JSExportsPhase {
      ...
      val body = tree match {
        ...
        rhs) =>
          ...
          withScopedVars(
            (initialThis match {
              case This(_)  =>
                collection.Seq(methodTailJumpThisSym := thisDef.symbol,
                  fakeTailJumpParamRepl := (NoSymbol, NoSymbol))
              case Ident(_) =>
                collection.Seq(methodTailJumpThisSym := NoSymbol,
    ...
    def genAsInstanceOf(value: js.Tree, to: Type)(
      ...
      if (sym == ObjectClass || isRawJSType(to)) {
         ...
         */
        value
      } else if (FunctionClass.collection.Seq contains to.typeSymbol) {
    ...
    private def genApplyDynamic(tree: ApplyDynamic): js.Tree = {
      ...
      if (isEqOrNeq) {
        ...
        for {
          (rtClass, reflBoxClass) <- collection.Seq(
    ...
    /** Gen actual actual arguments to a primitive JS call.
      ...
      *  collection.Seq is passed to a varargs parameter with the syntax `collection.Seq: _*`) will be
    ...
    /** Gen JS code for a repeated param of a primitive JS method
      *  In this case `arg` has type collection.Seq[T] for some T, but the result should
      ...
      *  compile-time.
      *  Otherwise, it returns a JSSpread with the collection.Seq converted to a js.Array.
      */
    ...
    object WrapArray {
      lazy val isWrapArray: Set[Symbol] = collection.Seq(
        nme.wrapRefArray,
        nme.wrapByteArray,
        nme.wrapShortArray,
        nme.wrapCharArray,
  ...
  private lazy val HijackedNumberClasses =
    collection.Seq(BoxedByteClass, BoxedShortClass, BoxedIntClass, BoxedLongClass,
      BoxedFloatClass, BoxedDoubleClass)
  private lazy val HijackedBoxedClasses =
    collection.Seq(BoxedUnitClass, BoxedBooleanClass) ++ HijackedNumberClasses

...
object Ast {
  ...
  sealed class Block(val leftBracket: String, val rightBracket: String, val values: collection.Seq[ComponentValue])
    extends ComponentValue
  sealed case class BracketsBlock(override val values: collection.Seq[ComponentValue]) extends Block("(", ")", values)
  sealed case class CurlyBracketsBlock(override val values: collection.Seq[ComponentValue]) extends Block("{", "}", values)
  sealed case class SquareBracketsBlock(override val values: collection.Seq[ComponentValue]) extends Block("[", "]", values)
  ...
  sealed case class AttributeSelector(name: Option[String],
                                      attrs: collection.Seq[(String, Option[String], Option[String])]) extends PartSelector
  ...
  sealed case class PseudoSelectorPart(pseudoClass: String, param: collection.Seq[ComponentValue]) extends ComplexSelectorPart
  sealed case class ComplexSelector(firstPart: Option[PartSelector],
                                    parts: collection.Seq[ComplexSelectorPart]) extends SingleSelector

  sealed case class MultipleSelector(firstSelector: SingleSelector,
                                     selectors: collection.Seq[(String, SingleSelector)]) extends Selector

  sealed abstract class Rule

  sealed case class QualifiedRule(selector: Either[Selector, collection.Seq[ComponentValue]],
                                  block: DeclarationList) extends Rule
  sealed case class AtRule(name: String, options: collection.Seq[ComponentValue],
                           block: Option[Either[DeclarationList, RuleList]]) extends Rule
  sealed case class RuleList(rules: collection.Seq[Rule])

  sealed case class Stylesheet(rules: collection.Seq[Either[Rule, CToken]])

  sealed case class Declaration(name: String, value: collection.Seq[ComponentValue], isImportant: Boolean)
  sealed case class DeclarationList(declarations: collection.Seq[Either[Declaration, AtRule]])
}
